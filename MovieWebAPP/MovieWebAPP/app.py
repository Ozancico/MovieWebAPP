...
def home():
    ...
    for title in page_titles:
        # Search for the movie in the DB (global, not filtered by user)
        db_movie = None
        # Get all movies and search for an exact title match
...
def movie_reviews(movie_id):
    ...
    return render_template('reviews.html', reviews=reviews, movie_id=movie_id, back_url=back_url)
...
def search():
    """
    Search for movies in the database (by title, director, year). If no results, optionally query OMDb.
    """
    query = request.args.get('q', '').strip()
    results = []
    omdb_result = None
